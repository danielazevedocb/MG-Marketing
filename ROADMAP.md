...
As 14 changes acima estão **arquivadas** (`openspec/changes/archive/`) — o escopo v1 planejado foi
implementado. `openspec/specs/` mantém os requisitos testáveis vivos por domínio para consulta.

Não há change ativa no momento. Próximos passos devem ser abertos como novas changes do OpenSpec
(`openspec proposal ...`) conforme surgirem — este roadmap não cobre trabalho pós-v1 (hardening,
performance, débito técnico), que é rastreado via revisão de código e issues, não via change.
